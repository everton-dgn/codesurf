...
- **Relay test import bug.** `packages/contex-relay/src/runtime.test.ts` imported `ContexRelay` from `./types`, but it's exported from `./relay` (vitest strips the type-only import so the suite passed, masking it). Split the import. tsgo −1. (148→145, relay suite still 40 pass.)
